...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        // STUDENT WORK...
	// allocate block of pointers (save pointer and end pointer) 
	block_pointers = new value_type*[bp_array_size];
	block_pointers_end = block_pointers + bp_array_size - 1; // if bp array size is 5, first pointer is x, 2nd x+1, 3rd x+2, 4th x+3, 5th/END x+4
		...
		if(source.block_pointers + bp_array_index == source.first_bp){
			first_bp = block_pointers+bp_array_index;
                ...
                {
                    // STUDENT WORK...
			value_type * src_item_ptr = source.block_pointers[bp_array_index]+block_item_index;
			value_type * new_item_ptr = block_pointers[bp_array_index]+block_item_index;
			if(src_item_ptr == source.front_ptr){front_ptr = new_item_ptr;}
			if(src_item_ptr == source.back_ptr){back_ptr = new_item_ptr;}
    ...
    void deque<Item>::clear () {
        ...
        for(value_type ** cursor = first_bp; cursor <= last_bp; ++cursor){
            value_type * current_block = *cursor;
